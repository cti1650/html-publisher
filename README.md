...
※ APIキーを提示しない場合、書き込み系API（POST/PUT）は揮発モード（cache）固定で動作します。永続モード（Gist）への書き込みには認証が必要です。
...
| 状態 | 挙動 |
|---|---|
| **認証済み**（正しいキー） | 揮発・永続両モード使用可、Gist 書き込み・更新可 |
| **匿名**（キー未提示） | **揮発モード固定**。Gist 作成/更新/import は拒否 (403)。読み取り系は public |
| **拒否**（キー不正） | 401 |

つまり、公開デプロイで誰でも揮発モードのツールは作れる一方、永続 Gist への書き込みはキー保持者のみという 2 段階の権限分離が自動的に効きます。
...
APIキー無しで接続した場合は、自動的に揮発モード（cache）固定で動作します。
**作成されるツールは「最終アクセスから約5時間」で自動削除されます。**永続したい場合は API キー認証付きで使ってください。
匿名で `import_gist` や Gist ID の更新を試みると 403 エラーになります。
